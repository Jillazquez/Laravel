                    ...
                    <div>
                        <div class="overflow-x-auto h-60">
                            <table class="table table-xs table-pin-rows table-pin-cols">
                                ...
                                <tbody>
                                @php
                                $idiomas = collect(config("idiomas"))->sortByDesc(fn($idioma)=>
                                $alumno->idiomas->contains("idioma",$idioma))->values();
                                @endphp
                                @foreach($idiomas as $idioma)
                                    <tr>
                                        <td>
                                            <input type="checkbox" value="{{$idioma}}" 
                                            name="idiomas[]" {{$alumno->idiomas->contains("idioma",$idioma) ? "checked" : ""}} >
                                                {{$idioma}}
                                        </td>
                                        <td>
